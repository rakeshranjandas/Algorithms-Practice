...
struct EulerTourLCA {
	...
	EulerTourLCA(VVI &g, int n) {
		last.resize(n+1);		
		dfs(g, 1, 0, 0);
		st.setV(depth);
	}

	void dfs(VVI &g, int cur, int par, int d) {
		tour.pb(cur);
		last[cur] = SZ(tour)-1;
		depth.pb(d);
...
void solve() {
	...
	auto eulerLCA = EulerTourLCA(g, n);
	while (q--) {
		int a, b;
		cin >> a >> b;
		cout << eulerLCA.getLCA(a, b) << endl;
	}
}
